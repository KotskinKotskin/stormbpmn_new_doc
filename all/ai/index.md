...
Если на каком-то из этапов генерации диаграммы случилась ошибка или AI-помощник сгенерировал не то, что ожидалось, можно откатить изменения и вернуться к любому из шагов создания модели. Для этого перейдите на вкладку {{ plantuml.history }}:

![]({{ media.screenshots.all_models }}/{{ media.actions.ai_create_plantuml }}/ai_create_plantuml_step_05.png)

Будьте осторожны и внимательны! При нажатии кнопки {{ plantuml.del_history }} история изменений диаграммы будет безвозвратно потеряна без возможности её восстановления.