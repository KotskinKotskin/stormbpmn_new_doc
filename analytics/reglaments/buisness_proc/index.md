...
1. `{{` `participantsTable` `}}` - таблица участников. Колонки:
...
1. `{{` `assigneesListTable` `}}` - таблица исполнителей процессов. Колонки:
...
1. `{{` `processCollaboration` `}}` - таблица связей процесса. Колонки:
...
1. {{ processAssets }} - таблица элементов архитектуры процесса (без повторений). Колонки:
...
1. `{{` `processAssetsToActivity` `}}` - таблица элементов архитектуры процесса в привязке к задачам. Колонки:
...
1. {{ processApprovals }} - таблица согласования процесса. Колонки:
