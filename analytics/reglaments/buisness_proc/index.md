...
`{{ <span>onlineVersionUrl</span> }}` - ссылка на версию процесса в онлайне
<code>{{ <span>processDescription</span> }}</code> - описание процесса


### Табличные теги

1. `{{` `participantsTable` `}}` -- таблица участников. Доступные колонки:
