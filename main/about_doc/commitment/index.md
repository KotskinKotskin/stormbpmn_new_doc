...
- `vars.yaml` — переменные документации для текущей локали. Синтаксис переменных в текстах: `{{ some_section.some_var }}`.
...
В файле `.meta.yml` обязательно должен быть прописан `title:`, он используется в качестве `H1` в `index.md` — `{{ page.meta.title }}`.
